...
Extracts the ISO's `casper/minimal.squashfs` into `build/inst/root/`, injects the custom kernel + modules + firmware + DTB, then chroots (via `qemu-aarch64-static` + binfmt) to configure apt for the arm64 ports mirror, create user `myuser` (password `surface`), and set `graphical.target` as default (GDM3 shows the normal login prompt).
